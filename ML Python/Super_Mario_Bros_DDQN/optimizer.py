...
class Optimizer:
    ...
    def step(self, state, action, reward, next_state, done):
        self.memory.add( state.T, action, reward, next_state.T, done )
    ...
    def _learn(self):                    
        states, actions, rewards, next_states, dones, importance, sample_indices = self.memory.sample()
        ...
        with torch.no_grad():
            ...
            # ie_rewards = torch.clamp( rewards +  ( Ri.detach() * 0.001 ), -15, 15 )
