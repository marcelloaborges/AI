...
from model import ActorCriticModel, RNDTargetModel, RNDPredictorModel
...
CHECKPOINT_RND_GOALIE = './checkpoint_rnd_goalie.pth'
CHECKPOINT_STRIKER = './checkpoint_striker.pth'
CHECKPOINT_RND_STRIKER = './checkpoint_rnd_striker.pth'
...
# NEURAL MODEL
goalie_actor_critic_model = ActorCriticModel( goalie_state_size, goalie_action_size ).to(DEVICE)
goalie_rnd_target_model = RNDTargetModel( goalie_state_size, goalie_action_size ).to(DEVICE)
goalie_rnd_predictor_model = RNDPredictorModel( goalie_state_size, goalie_action_size ).to(DEVICE)
goalie_optim = optim.Adam( list( goalie_actor_critic_model.parameters() ) + list( goalie_rnd_predictor_model.parameters() ), lr=GOALIE_LR )


striker_actor_critic_model = ActorCriticModel( striker_state_size, striker_action_size ).to(DEVICE)
striker_rnd_target_model = RNDTargetModel( striker_state_size, striker_action_size ).to(DEVICE)
striker_rnd_predictor_model = RNDPredictorModel( striker_state_size, striker_action_size ).to(DEVICE)
striker_optim = optim.Adam( list( striker_actor_critic_model.parameters() ) + list( striker_rnd_predictor_model.parameters() ), lr=STRIKER_LR )

goalie_actor_critic_model.load( CHECKPOINT_GOALIE )
goalie_rnd_predictor_model.load( CHECKPOINT_RND_GOALIE )
striker_actor_critic_model.load( CHECKPOINT_STRIKER )
striker_rnd_predictor_model.load( CHECKPOINT_RND_STRIKER )


# AGENTS
goalie_0 = Agent( DEVICE, GOALIE_0_KEY, goalie_actor_critic_model, N_STEP )
goalie_1 = Agent( DEVICE, GOALIE_1_KEY, goalie_actor_critic_model, N_STEP )
goalie_optimizer = Optimizer( DEVICE, goalie_actor_critic_model, goalie_rnd_target_model, goalie_rnd_predictor_model, goalie_optim,  
    N_STEP, BATCH_SIZE, EXT_GAMMA, EXT_COEF, INT_GAMMA, INT_COEF, EPSILON, ENTROPY_WEIGHT, GRADIENT_CLIP)

striker_0 = Agent( DEVICE, STRIKER_0_KEY, striker_actor_critic_model, N_STEP )
striker_1 = Agent( DEVICE, STRIKER_1_KEY, striker_actor_critic_model, N_STEP )
striker_optimizer = Optimizer( DEVICE, striker_actor_critic_model, striker_rnd_target_model, striker_rnd_predictor_model, striker_optim,  
    N_STEP, BATCH_SIZE, EXT_GAMMA, EXT_COEF, INT_GAMMA, INT_COEF, EPSILON, ENTROPY_WEIGHT, GRADIENT_CLIP)
...
def ppo_train():
    ...
    for episode in range(n_episodes):
        ...
        while True:       
            ...
            # random            
            # action_goalie_0 = np.random.choice(goalie_action_size)
            # action_striker_0 = np.random.choice(striker_action_size)
