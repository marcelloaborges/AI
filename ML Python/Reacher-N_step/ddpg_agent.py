...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        self.n_step_memory = NStepBuffer()
    ...
    def step(self, state, action, reward, next_state, done):        
        ...
        self.n_step_memory.add(state, action, reward, next_state, done)        

        self.R = ( self.R + np.array(reward) * GAMMA_N ) / GAMMA
        
        if np.any( done ):
            while len(self.n_step_memory) > 0:
                n = len(self.n_step_memory)
                state, action, reward, next_state, done = self.n_step_memory.sample(n, self.R)
                self.memory.add(state, action, reward, next_state, done)

                self.R = self.R - self.n_step_memory.memory[0].reward
                self.n_step_memory.memory.pop(0)

            self.R = np.zeros(20)

        if len(self.n_step_memory) >= N_STEPS:
            state, action, reward, next_state, done = self.n_step_memory.sample(N_STEPS, self.R)
            self.memory.add(state, action, reward, next_state, done)
    ...
    def learn(self, experiences):
        ...
        # Q_targets = rewards + (GAMMA_N * Q_targets_next * (1 - dones))
        # Compute critic loss
...
class NStepBuffer:
    ...
    def sample(self, batch_size, R):
        ...
        state, action, _, _, _ = self.memory[0]
        _, _, _, next_state, done = self.memory[batch_size - 1]
